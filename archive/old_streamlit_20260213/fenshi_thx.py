...
iFinD = iFinD()
iFinD.login("shrhqy002","P2kNCM66")
...
def load_stock_list_from_thx(thx_file="thx.txt"):
    """从 thx.txt 文件中解析股票代码列表

    文件格式示例：
        THS_RQ('688001.SH,688002.SH,...','tradeDate;...','','format:list')
    """
    if not os.path.exists(thx_file):
        print(f"\n❌ 未找到股票列表文件 {thx_file}")
        return []

    try:
        with open(thx_file, 'r', encoding='utf-8') as f:
            content = f.read()

        match = re.search(r"THS_RQ\('([^']+?)'\s*,", content, re.S)
        if not match:
            print(f"\n❌ 无法从 {thx_file} 解析股票代码")
            return []

        codes_str = match.group(1)
        codes = [code.strip() for code in codes_str.replace('\n', '').split(',') if code.strip()]

        if not codes:
            print(f"\n⚠️  {thx_file} 中未找到有效股票代码")
        else:
            print(f"\n✅ 从 {thx_file} 读取到 {len(codes)} 只股票")
...
def main():
    ...
    if saved_stocks and len(saved_stocks) > 0:
        ...
        # 首次运行：从 thx.txt 读取股票列表
        print(f"\n💡 首次运行，将从 thx.txt 读取股票列表")
        stock_list = load_stock_list_from_thx("thx.txt")

        if not stock_list:
            print(f"\n⚠️  未能从 thx.txt 读取到股票，将尝试通过 baostock 获取")
        ...
        if not stock_list or len(stock_list) == 0:
            print(f"\n❌ 未能获取到股票列表")
            print(f"   可能原因：")
            print(f"   1. thx.txt 文件内容格式错误或为空")
            print(f"   2. 网络连接问题")
            print(f"   3. baostock 服务器故障")
            print(f"   4. 选择的日期不是交易日")
            print(f"\n请检查 thx.txt 内容、网络连接和日期设置后重试")
            return
        
        print(f"✅ 获取到股票数量：{len(stock_list)} 只")

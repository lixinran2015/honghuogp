      ...
      <div class="xl:col-span-8 space-y-4">
        ...
        <div class="bg-white rounded-lg border border-border p-4">
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex items-center gap-2">
              <span v-if="newHigh60dList.length > 0" class="text-xs text-warmgray-500">
                共 {{ newHigh60dList.length }} 只
              </span>
              <button
                @click="fetch60dHigh"
                :disabled="loading60dHigh"
              ...
              >
                <ArrowPathIcon class="w-3.5 h-3.5" :class="{ 'animate-spin': loading60dHigh }" />
                {{ loading60dHigh ? '计算中...' : '计算60日新高' }}
              </button>
            </div>
          </div>

          <div v-if="newHigh60dList.length > 0" class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-warmgray-50">
                  <th class="px-2 py-2 text-left text-warmgray-600 font-medium">股票</th>
                  <th class="px-2 py-2 text-left text-warmgray-600 font-medium">板块</th>
                  <th class="px-2 py-2 text-right text-warmgray-600 font-medium">收盘价</th>
                  <th class="px-2 py-2 text-right text-warmgray-600 font-medium">涨幅</th>
                  <th class="px-2 py-2 text-right text-warmgray-600 font-medium">突破幅度</th>
                  <th class="px-2 py-2 text-right text-warmgray-600 font-medium">60日最高</th>
                  <th class="px-2 py-2 text-right text-warmgray-600 font-medium">成交额</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  v-for="stock in newHigh60dList"
                  :key="stock.ts_code"
                ...
                >
                  ...
                  <td class="px-2 py-2 text-xs text-warmgray-600">{{ stock.industry || '-' }}</td>
                  <td class="px-2 py-2 text-right font-medium">{{ stock.close?.toFixed(2) }}</td>
                  <td class="px-2 py-2 text-right" :class="stock.change_pct >= 0 ? 'text-profit' : 'text-loss'">
                    {{ stock.change_pct > 0 ? '+' : '' }}{{ stock.change_pct?.toFixed(2) }}%
                  </td>
                  <td class="px-2 py-2 text-right font-medium text-cta">
                    {{ stock.break_pct?.toFixed(2) }}%
                  </td>
                  <td class="px-2 py-2 text-right text-warmgray-600">{{ stock.high_60d?.toFixed(2) }}</td>
                  <td class="px-2 py-2 text-right text-warmgray-600">
                    {{ stock.amount >= 10000 ? (stock.amount / 10000).toFixed(1) + '亿' : stock.amount?.toFixed(0) + '万' }}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div v-else-if="!loading60dHigh" class="text-center py-6 text-warmgray-400 text-sm">
            点击上方按钮计算60日新高股票
          </div>
        </div>
...
const newHigh60dList = ref([])
const loading60dHigh = ref(false)
...
async function fetch60dHigh() {
  loading60dHigh.value = true
  try {
    const response = await fetch('/api/short-term/screening/60d-high?limit=100')
    const data = await response.json()
    if (data.success) {
      newHigh60dList.value = data.data || []
    } else {
      console.error('60日新高计算失败:', data.error)
      newHigh60dList.value = []
    }
  } catch (e) {
    console.error('请求60日新高失败:', e)
    newHigh60dList.value = []
  } finally {
    loading60dHigh.value = false
  }
}

  ...
  <div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-4 bg-white rounded-lg shadow p-4">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-2">
            <label class="text-sm text-gray-600 font-medium">榜单日期：</label>
            <input
              v-model="popularityDate"
              type="date"
              class="px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            :title="eligibleStocksCount === 0 ? '没有符合条件的股票（条件：首日入榜 且 60日新高）' : `将 ${eligibleStocksCount} 只符合条件的股票加入跟踪池`"
          >
    ...
    <div v-if="stockDate && !loading" class="mb-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
      <p class="text-sm text-blue-800">
        ...
        将使用 <span class="font-semibold">{{ popularityDate }}</span> 的人气榜前 {{ maxRank }} 名股票，计算这些股票在 <span class="font-semibold">{{ stockDate }}</span> 是否第一次突破60日新高
      </p>
    </div>
          ...
          <thead class="bg-gray-50">
            <tr>
              ...
              <th 
                ...
                @click="sortByFirstEntry"
              >
                <div class="flex items-center justify-center gap-1">
                  <span>首次入榜</span>
                  <svg 
                    v-if="sortField === 'is_first_entry'"
                    class="w-4 h-4" 
            ...
            >
              ...
              <td class="px-4 py-3 whitespace-nowrap text-center">
                <span 
                  ...
                  :title="item.first_entry_date ? `首次入榜日期: ${item.first_entry_date}` : '首次入榜单'"
                >
                  新
                </span>
                <span v-else class="text-sm text-gray-400">-</span>
    ...
    <div v-if="!loading && data.length > 0" class="mt-4 bg-white rounded-lg shadow p-4">
      <div class="flex items-center gap-6 text-sm">
        <div>
          <span class="text-gray-600">找到股票：</span>
          <span class="font-semibold text-blue-600">{{ data.length }}</span>
          <span class="text-gray-500">只</span>
        </div>
        <div>
          <span class="text-gray-600">人气榜范围：</span>
          <span class="font-semibold text-gray-900">前{{ popularityCount || maxRank }}名</span>
        </div>
        <div>
        ...
        <div>
          <span class="text-gray-600">榜单日期：</span>
          <span class="font-semibold text-gray-900">{{ popularityDate || '今天' }}</span>
        </div>
      </div>
    </div>
  </div>
...
const data = ref([])
const stockDate = ref('') // 股票日期：用于计算股票是否突破60日新高
const popularityDate = ref('') // 榜单日期：用于获取人气榜数据
const maxRank = ref(100)
const popularityCount = ref(0)
const dataSource = ref(null) // 'calculated' 或 'queried'

// 排序相关
const sortField = ref(null)
const sortOrder = ref('desc')

// 计算符合条件的股票数量（首日入榜 且 60日新高）
const eligibleStocksCount = computed(() => {
  if (!data.value || data.value.length === 0) {
    return 0
  }
  return data.value.filter(stock => {
    return stock.is_first_entry === true && stock.is_60d_high === true
  }).length
})
...
const sortedData = computed(() => {
  ...
  const sorted = [...data.value].sort((a, b) => {
    ...
    // 特殊处理：首次入榜（布尔值排序）
    if (sortField.value === 'is_first_entry') {
      // 处理 null/undefined 值，放在最后
      if (aVal === null || aVal === undefined) return 1
      if (bVal === null || bVal === undefined) return -1
      
      // 转换为数字：true=1, false=0
      const aNum = aVal ? 1 : 0
      const bNum = bVal ? 1 : 0
      
      if (sortOrder.value === 'asc') {
        return aNum - bNum  // 首次入榜的在后
      } else {
        return bNum - aNum  // 首次入榜的在前
      }
    }
...
// 从数据库查询
async function queryFromDatabase(silent = false, autoCalculateIfEmpty = false) {
  if (!stockDate.value) {
    if (!silent) {
      alert('请选择股票日期')
    }
  ...
  try {
    const params = {
      ...
      is_first_60d_high: true  // 只查询第一次突破60日新高的股票
    }
...
async function calculate(silent = false) {
  ...
  try {
    const params = {
      trade_date: stockDate.value,
      popularity_date: popularityDate.value,
      max_rank: maxRank.value
    }
...
async function addAllToWatchlist() {
  ...
  // 筛选符合条件的股票：首日入榜 且 60日新高
  const eligibleStocks = data.value.filter(stock => {
    return stock.is_first_entry === true && stock.is_60d_high === true
  })
  ...
  if (!confirm(`确认要将 ${eligibleStocks.length} 只符合条件的股票加入跟踪池吗？\n条件：首日入榜 且 60日新高\n理由：60日新高`)) {
    return
  }
  ...
  try {
    ...
    // 显示结果
    let message = `添加完成！\n符合条件的股票: ${eligibleStocks.length} 只\n成功: ${successCount} 只`
    if (failCount > 0) {
      message += `\n失败: ${failCount} 只`
...
// 初始化：设置默认日期为今天，并自动查询今天的数据，如果没有数据则自动计算
onMounted(async () => {
  ...
  popularityDate.value = todayStr
  
  // 只查询缓存数据，不自动计算（静默模式，不弹窗提示）

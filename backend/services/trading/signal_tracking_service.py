...
class SignalTrackingService:
    ...
    def generate_signals(self, trade_date: date) -> int:
        ...
        # 2. 自动识别情绪周期
        emotion_cycle = detect_emotion_cycle(trade_date, self.warehouse)
    ...
    def _get_close_prices(self, ts_codes: List[str], trade_date_str: str) -> Dict[str, float]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                for tc in ts_codes:
                    # 标准化代码：去掉交易所后缀
                    normalized = tc.replace('.SH', '').replace('.SZ', '').replace('.BJ', '')
                    code_lookup[normalized] = tc
    ...
    def _update_single_signal(self, sig: ShortTermSignalTracking, lookup: Dict[tuple, Any], trade_date: date) -> int:
        ...
        for i, high in enumerate(highs):
            if high > max_price:
                max_price = high
                peak_idx = i
        if max_price > entry_price and peak_idx < len(lows):
            min_price_after_peak = min(lows[peak_idx:])
            sig.max_drawdown = round((min_price_after_peak - max_price) / max_price, 4)
        else:
            sig.max_drawdown = 0.0
